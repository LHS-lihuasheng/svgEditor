...
import { KeyframesSplinePreview } from "./animation/KeyframesSplinePreview";
import {
  Collapsible,
  CollapsibleContent,
  CollapsibleTrigger,
...
// 内部上下文状态管理
function useAnimationEditorState(component: BaseComponent, componentId: string, onUpdateProperty: (path: string, value: any) => void) {
  // 只读取组件的现有值，不设置默认值
  ...
  const keyframes = useMemo(() => {
    ...
    return valuesList.map((val: any, index: any) => ({
      value: val,
  ...
  const updateKeyframes = useCallback((newKeyframes: Keyframe[]) => {
    ...
    onUpdateProperty('attributes.values', newValues);
    onUpdateProperty('attributes.keyTimes', newTimes);
  }, [onUpdateProperty]);
  ...
  const updateSplines = useCallback((newSplines: Spline[]) => {
    ...
    // 直接调用更新函数
    onUpdateProperty('attributes.keySplines', newSplinesStr);
  }, [onUpdateProperty]);
  ...
  const removeKeyframe = useCallback((index: number) => {
    ...
    const newKeyframes = keyframes.filter((_: any, i: number) => i !== index);

    // 重新计算时间，使之均匀分布
    newKeyframes.forEach((kf: Keyframe, i: number) => {
    ...
    if (calcMode === "spline" && splines.length > 0) {
      ...
      if (index < newSplines.length) {
        newSplines = newSplines.filter((_: any, i: number) => i !== index);
        updateSplines(newSplines);
      }
    }
  }, [keyframes, splines, calcMode, updateKeyframes, updateSplines]);
  ...
  const editorState = useAnimationEditorState(
    ...
    (path, value) => {
      ...
      if (path === 'animationMode') {
        ...
        if (propName) {
          updateComponentAttribute(selectedComponent.id, propName, value);
        }
      }
    }
  );
                  ...
                  <div className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <div className="space-y-2 mt-2">
                        {editorState.keyframes.map((keyframe: Keyframe, index: number) => (
                      ...
                      <Collapsible>
                        ...
                        <CollapsibleContent className="p-2">
                          <div className="space-y-4">
                            ...
                            {editorState.splines.map((spline: Spline, index: number) => (
